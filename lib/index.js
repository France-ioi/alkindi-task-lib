...
export function hostTask (container, options, TaskBundle) {
  ...
  if (options.task) {
    ...
    // Notify the parent that we are ready to load the task.
    window.parent.postMessage(JSON.stringify({task: "ready"}), "*");
  }
  if (options.view) {
