...
export const Dict = EpicComponent(self => {
   /* Props:
         grid
         renderCell
   */
   self.render = function () {
      const {dict, renderValue} = self.props;
      if (!dict) {
         return <span className="code-error">None</span>;
      }
      const items = [];
      Object.keys(dict).forEach(function (key) {
         items.push(`"${key}": ${renderValue(dict[key])}`);
      });
...
export default {StrLit, Var, Assign, Call, Dict, Grid};
