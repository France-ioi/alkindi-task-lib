...
module.exports = function (config) {
  ...
  app.get('/', function (req, res) {
    const options = {
      view: 'task',
      taskBaseUrl: process.env.BASE_URL || ''
    };
    if (process.env.MODE !== 'passive') {
      // TODO: get params, seed from query string
      const params = req.query.p ? JSON.parse(req.query.p) : {};
      const seed = req.query.seed || "";
      config.generate(params, seed, function (err, genOutput) {
        if (err) {
          return res.status(500).send(`generation failed: ${err}`);
        }
        options.initialState = genOutput;
        sendResponse();
      })
    } else {
      sendResponse();
  ...
  app.post('/grantHint', function (req, res) {
    const {full_task, task, query} = req.body;
    config.grantHint(full_task, task, query, function (err, result) {
      if (err) {
        return res.status(500).json({success: false, error: err.toString()});
      }
      res.json(result);
