...
export default function WorkspaceBuilder (impl) {
  ...
  return function* (deps) {
    ...
    yield defineSelector('WorkspaceSelector', function (state) {
       const {score, feedback, task, workspace} = state;
       console.log('feedback', score, feedback);
       return {score, feedback, task, workspace};
    });
    yield defineView('Workspace', 'WorkspaceSelector', EpicComponent(self => {

      self.render = function () {
        const {score, feedback, task, workspace, dispatch} = self.props;
        if (!workspace) {
          return <div>Workspace not loaded.</div>;
        }
        return <impl.View score={score} feedback={feedback} task={task} workspace={workspace} dispatch={dispatch}/>;
      };
